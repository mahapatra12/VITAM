...
const logAi = (level, message, onceKey = null) => {
    if (!debugAiLogs) {
        return;
    }
    if (onceKey) {
...
const askGroq = async (prompt, systemRole) => {
    if (!process.env.GROQ_API_KEY) {
        logAi("warn", "GROQ_API_KEY missing. Using rule-based fallback.", "groq-key-missing");
    ...
    try {
        ...
        if (status === 429) {
            logAi("warn", `Groq AI throttled [${status}] — degraded fallback active.`, "groq-rate-limit");
        } else {
            logAi("error", `Groq AI request failed [${status || "unknown"}]: ${msg}`);
        }
...
exports.graderAgent = async (assignmentData) => {
    ...
    const prompt = `Evaluate this assignment: ${toAiPayloadSnippet(assignmentData).text}`;
...
exports.analyticsAgent = async (data, metric) => {
    ...
    const prompt = `Analyze ${metric} trends from this data: ${toAiPayloadSnippet(data).text}. Identify risks and areas for improvement.`;
    return await askGroq(prompt, systemRole);
};
...
exports.resumeAgent = async (studentProfile) => {
    ...
    const prompt = `Build a skeleton and suggestions for a resume based on this profile: ${toAiPayloadSnippet(studentProfile).text}`;
    return await askGroq(prompt, systemRole);
};
...
exports.predictFailure = async (studentData) => {
    ...
    const prompt = `Analyze this student data for academic risk: ${toAiPayloadSnippet(studentData).text}. Predict risk level (LOW / MEDIUM / HIGH) and provide specific reasons.`;
    return await askGroq(prompt, systemRole);
};
...
exports.careerGuide = async (studentData) => {
    ...
    const prompt = `Based on this student's branch, interests, and current skills: ${toAiPayloadSnippet(studentData).text}, suggest the best career domain, a 6-month roadmap, and essential skills to learn.`;
    return await askGroq(prompt, systemRole);
};
...
exports.financeAgent = async (financeData) => {
    ...
    const prompt = `Analyze this institutional financial data: ${toAiPayloadSnippet(financeData).text}. Highlight critical areas (e.g., pending fees), and suggest exactly 3 actionable steps to optimize cash flow and revenue. Format the output in clean text with bullet points.`;
    return await askGroq(prompt, systemRole);
};
...
exports.academicAgent = async (academicData) => {
    ...
    const prompt = `Analyze this college-wide academic telemetry: ${toAiPayloadSnippet(academicData).text}. Identify the worst-performing department and suggest 2 targeted interventions to improve their overall pass rate and attendance footprint. Format the output in clean text.`;
    return await askGroq(prompt, systemRole);
};
...
exports.strategyAgent = async (institutionalData) => {
    ...
    const prompt = `Review this high-level institutional snapshot: ${toAiPayloadSnippet(institutionalData).text}. Generate a cohesive "State of the Institution" report. Identify the single biggest bottleneck to institutional growth (financial or academic) and mandate an executive priority list for the upcoming quarter.`;
    return await askGroq(prompt, systemRole);
};
