...
const Navbar = ({ title = 'Dashboard', onMenuClick, onCommsClick }) => {
  ...
  return (
    <header
      className="fixed inset-x-0 top-0 z-[100] border-b border-white/5 bg-slate-950/80 backdrop-blur-xl lg:left-[18rem]"
      style={{ paddingTop: 'max(env(safe-area-inset-top), 0px)' }}
    >
      <div
        className="mx-auto flex h-16 items-center justify-between gap-3 px-3 sm:px-6"
        style={{
      ...
      >
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          <button
            ...
            className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-slate-200 transition-colors hover:bg-white/10 hover:text-white lg:hidden"
          >
            <Menu size={18} />
          </button>
          ...
          <div className="min-w-0">
            ...
            <p className="hidden truncate text-[11px] uppercase tracking-wide text-slate-400 sm:block">
              VITAM Platform
            </p>
          </div>
        </div>
        ...
        <div className="hidden flex-1 justify-center xl:flex">
          <button
            ...
            className="flex w-full max-w-md items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-left text-slate-300 transition-all hover:border-appleBlue/40 hover:bg-white/10"
          >
            <Search size={15} />
            <span className="flex-1 text-sm">Search pages, actions, users</span>
            <span className="rounded border border-white/15 px-2 py-0.5 text-[11px] text-slate-300">Ctrl+K</span>
          </button>
        </div>

        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
          <div className="hidden items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-slate-300 lg:flex">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="relative inline-flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-slate-200 transition-colors hover:bg-white/10 hover:text-white"
              >
                <Bell size={16} />
                {unreadCount > 0 && <span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-appleBlue" />}
              </button>

              {showNotif && (
                <div className="absolute right-0 top-12 z-20 w-[min(20rem,calc(100vw-1.5rem))] rounded-2xl border border-white/10 bg-slate-900/95 p-2 shadow-2xl backdrop-blur-2xl">
                  <div className="mb-2 border-b border-white/10 px-3 py-2 text-xs font-semibold text-slate-300">
                    Notifications
                  </div>
                  <div className="custom-scrollbar max-h-72 overflow-y-auto p-1">
                    {notifications.map((n) => (
                      <button
                        key={n.id}
                        type="button"
                        className="mb-1 w-full rounded-xl px-3 py-2 text-left hover:bg-white/5"
                      >
          ...
          <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-2 py-1">
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-appleBlue to-applePurple text-xs font-bold text-white shadow-apple-glow">
